...
void oldtriangle (
	...
	float z12, z13, z23, z12_step, z13_step, z23_step;
	float z_left, z_right, z_step;
	float z, z_temp;
	...
	if (flag & DRAW1) {
		...
		z12_step=float (z2-z1)/float(y2-y1);
	}
	f_x13_step=float(x3-x1)/float(y3-y1);
	z13_step=float(z3-z1)/float(y3-y1);
	if (flag & DRAW2) {
		f_x23_step=float(x3-x2)/float(y3-y2);
		z23_step=float(z3-z2)/float(y3-y2);
	}

	int xm=x1+f_x13_step*float(y2-y1);
	float zm=z1+z13_step*float(y2-y1);
