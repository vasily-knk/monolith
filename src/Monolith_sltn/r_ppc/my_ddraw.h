...
/*#define GETZBUFFER(x,y) ( ((x)>=r_x1&&(x)<r_x2&&(y)>=r_y1&&(y)<r_y2) ? z_buffer[(y)*dd_width+(x)] : 0)
#define SETZBUFFER(x,y,z) if((x)>=r_x1&&(x)<r_x2&&(y)>=r_y1&&(y)<r_y2)z_buffer[(y)*dd_width+(x)]=z*/
#define GETZBUFFER(x,y) z_buffer[(y)*dd_width+(x)]
#define SETZBUFFER(x,y,z) z_buffer[(y)*dd_width+(x)]=(z)
#endif